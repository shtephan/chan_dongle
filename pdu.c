...
EXPORT_DEF const char * pdu_parse(char ** pdu, size_t tpdu_length, char * oa, size_t oa_len, str_encoding_t * oa_enc,
	char ** msg, str_encoding_t * msg_enc, int* msg_ref, int* msg_parts, int* msg_part)
{
	const char * err = NULL;
										...
										if((size_t)udl * 2 == pdu_length)
										{
											if(PDUTYPE_UDHI(pdu_type) == PDUTYPE_UDHI_HAS_HEADER)
											{
												...
												if(udhl >= 0)
												{
													...
													//ast_log (LOG_ERROR, "   pdu=%s\n   pdu_length=%1d udhl=%1d\n", *pdu,pdu_length,udhl);
													...
													if(pdu_length >= (size_t)(udhl * 2))
													{
														while (udhl >= 2) {
														  ...
														  /* subtract type and len bytes */
														  udhl -= 2;

														  //ast_log (LOG_ERROR, "   iei_type=%1d iei_len=%1d\n", iei_type,iei_len);
														  //by Stefan added if and then branch
														  if (iei_type==0){
